...
pub fn open_column_u128<T: MonotonicallyMappableToU128>(
    ...
    let (column_index_data, column_values_data) = body.split(column_index_num_bytes as usize);
    let column_index = crate::columnar::column_index::open_column_index(column_index_data)?;
    let column_values = crate::columnar::column_values::open_u128_mapped(column_values_data)?;
    Ok(Column {
        index: column_index,
        values: column_values,
    })
...
pub fn open_column_u128_as_compact_u64(bytes: OwnedBytes) -> io::Result<Column<u64>> {
    ...
    let column_index = crate::columnar::column_index::open_column_index(column_index_data)?;
    let column_values =
        crate::columnar::column_values::open_u128_as_compact_u64(column_values_data)?;
    Ok(Column {
        index: column_index,
        values: column_values,
    })

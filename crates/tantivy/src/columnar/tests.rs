...
use super::column_values::MonotonicallyMappableToU128;
use super::columnar::{ColumnType, ColumnTypeCategory};
use super::dynamic_column::{DynamicColumn, DynamicColumnHandle};
use super::value::{Coerce, NumericalValue};
use super::{
...
proptest! {
    ...
    fn test_columnar_merge_proptest(columnar_docs in proptest::collection::vec(columnar_docs_strategy(), 2..=3)) {
        ...
        super::merge_columnar(&columnar_readers_arr[..], &[], stack_merge_order, &mut output).unwrap();
...
fn test_columnar_merging_empty_columnar() {
    ...
    super::merge_columnar(
        &columnar_readers_arr[..],
        &[],
        super::MergeRowOrder::Stack(stack_merge_order),
        &mut output,
    )
    .unwrap();
    let merged_columnar = ColumnarReader::open(output).unwrap();
...
proptest! {
    ...
    fn test_columnar_merge_and_remap_proptest((columnar_docs, shuffle_merge_order) in columnar_docs_and_remap()) {
        ...
        super::merge_columnar(&columnar_readers_arr[..], &[], shuffle_merge_order.into(), &mut output).unwrap();
        let merged_columnar = ColumnarReader::open(output).unwrap();

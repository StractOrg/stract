...
fn compute_collapse_mapping_one(
    facet_terms: &mut crate::columnar::Streamer,
    facet_bytes: &[u8],
...
fn compute_collapse_mapping(
    facet_dict: &crate::columnar::Dictionary,
    facets: &BTreeSet<Facet>,
    ...
    let mut facet_terms: crate::columnar::Streamer = facet_dict.range().into_stream()?;
    if !facet_terms.advance() {
        return Ok(collapsed);
    }

...
impl CharNormalizer for UnicodeNFKC {
    fn normalize(&self, c: char) -> impl Iterator<Item = char> {
        unicode_normalization::UnicodeNormalization::nfkc(c)
    }

    fn should_normalize(&self, c: char) -> bool {
        match unicode_normalization::is_nfkc_quick(iter::once(c)) {
...
#[cfg(test)]
mod tests {
    use super::UnicodeNFKC;
    use crate::tokenizer::normalizer::Normalizer;
    use crate::tokenizer::Token;

    #[test]
    fn test_nfkc() {
        let normalizer = UnicodeNFKC;

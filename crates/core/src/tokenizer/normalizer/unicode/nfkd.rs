...
pub struct UnicodeNFKD;

impl CharNormalizer for UnicodeNFKD {
    fn normalize(&self, c: char) -> impl Iterator<Item = char> {
        unicode_normalization::UnicodeNormalization::nfkd(c)
    }

    fn should_normalize(&self, c: char) -> bool {
        match unicode_normalization::is_nfkd_quick(iter::once(c)) {
...
#[cfg(test)]
mod tests {
    use super::UnicodeNFKD;
    use crate::tokenizer::normalizer::Normalizer;
    use crate::tokenizer::Token;

    #[test]
    fn test_nfkd() {
        let normalizer = UnicodeNFKD;

...
It does this by waiting a certain amount of time between requests to the same domain. The waiting time is calculated by _min(max(fetchtime _ politeness, 1 sec), 60 sec)* where *fetchtime\* is the time it took to fetch the page. The crawler will wait at least 1 sec between requests and at most 60 seconds.

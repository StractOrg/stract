import {
  UnoGenerator,
  type UserConfig,
} from "https://esm.sh/@unocss/core@0.55.7";
  ...
  return {
    ...
    entrypoints: {
      "main": `
          ...
          import init from "https://esm.sh/@unocss/runtime@0.55.7";
    ...
    async renderAsync(ctx) {
      ...
      const { css } = await uno.generate(htmlText.replaceAll("[&amp;", "[&"));
      return {
        scripts: runtime ? [{ entrypoint: "main", state: {} }] : [],
        styles: [{
          cssText:
            `${unoResetCSS}\n*{border-color:rgba(229,231,235,var(--un-border-opacity,1))}\n${css}`,
        }],
      };
    },
  };

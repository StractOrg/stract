...
pub(crate) struct MemoryStore {
    adjacency: BTreeMap<NodeID, Vec<InternalEdge>>,
    node2id: BTreeMap<NodeName, NodeID>,
    ...
    fn outgoing_edges(&self, node: Node) -> Vec<Edge> {
        match self.node2id.get(&node.name) {
            ...
            Some(id) => self
                .adjacency
                .get(id)
                .expect("Fatal error: node2id and adjacency are out of sync")
                .clone()
                .into_iter()
                .map(|internal_edge| {
                    let to_name = self
